    ...
    <section className="panel p-6 sm:p-7">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="rounded-full bg-[#f7efe4] px-4 py-2 text-sm font-semibold text-slate-700">
          รออนุมัติ {pendingTrainerApplications.length} รายการ
        </div>
      </div>
          ...
          {pendingTrainerApplications.map((application) => (
            ...
            >
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                <button
                  type="button"
                  onClick={() => onApproveTrainer(application.id)}
                  className="rounded-full bg-[#123a35] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#0b2824]"
                >
                  อนุมัติเป็นเทรนเนอร์
                </button>
                <button
                  type="button"
                  onClick={() => onRejectTrainer(application.id)}
                  className="rounded-full border border-[#e3b4ae] px-5 py-3 text-sm font-semibold text-[#b42318] transition hover:bg-[#fff3f1]"
                >
                  ปฏิเสธคำขอ
                </button>
              </div>
